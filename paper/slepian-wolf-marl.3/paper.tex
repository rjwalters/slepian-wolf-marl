...
Two more recent (2024) works further extend the MI-regularization line. \textbf{MIR3}~\cite{liu2024mir3} applies a mutual-information regularizer between agents' histories and actions to bound a robustness gap, framing the IB as an explicit lower bound on worst-case-cooperative-partner reward. \textbf{MADPO}~\cite{madpo2024} \emph{maximizes} policy divergence between agents to enhance heterogeneity in shared-parameter regimes. Both share our broad prescription (information-theoretic terms in the MARL objective) without the distributed-source-coding framing or the explicit conditional-entropy budget.
...
\bibitem{madpo2024} Multi-agent divergence policy optimization (MADPO): Enhancing exploration and heterogeneity via mutual policy divergence (2024).
...
\emph{Heuristic motivation.} A teacher--student construction with maximum-likelihood estimation of $\pi_i$ from $(O_i, A_i^*)$ pairs, plus universal source coding arguments applied per agent, suggests that the conditional-entropy rate is achievable. A clean proof would require (a) showing the MLE concentrates at the right rate under non-i.i.d.\ data, and (b) connecting per-agent rate achievement to joint $J(\pi)$ via a continuity argument analogous to Conjecture~A.2. We do not develop either step and label this an achievability \emph{conjecture}, paired with the necessity conjecture A.2.
