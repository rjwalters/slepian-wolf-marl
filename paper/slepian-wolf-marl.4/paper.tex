...
%----------------------------------------------------------------------
\begin{abstract}
Multi-agent systems often need to coordinate without reliable communication---robot teams under jamming, distributed sensors that cannot flood the network. We propose viewing such cooperation through the lens of distributed source coding: each agent's policy acts as a lossy encoder of a latent optimal action distribution, and the environment evaluates whether agents' combined outputs cohere. The intuition comes from the Slepian--Wolf theorem (1973), which showed that observers with correlated data can compress as efficiently without coordination as with it. Our setting is sequential, lossy, and non-i.i.d., so Slepian--Wolf does not apply directly; we use it as a conceptual lens, not a derivation. The lens yields five testable predictions: per-agent capacity should scale with conditional entropy $H(A_i^* \mid A_{-i}^*)$ rather than $H(A_i^*)$; sample complexity should grow monotonically in the same quantity at fixed policy class; agents should specialize to avoid redundant encoding; communication helps only when local uncertainty exceeds what teammates' behavior reveals; and equal-conditional-entropy agents should be permutation-invariant in expectation. We state these as conjectures, propose a corresponding information-regularized objective using \emph{conditional} mutual information at the representation level, and describe an experimental protocol on a small firefighting environment (\emph{Bucket Brigade}) designed to test each prediction with bootstrap-CI estimators. Results from that protocol are deferred to a follow-up.
\end{abstract}
...
\begin{enumerate}
    \item \textbf{A unifying lens} linking distributed source coding to MARL, with random variables defined for state, observations, optimal actions, and learned actions, so that capacity, samples, specialization, communication, and symmetry all reduce to one quantity.
...
\paragraph{Important caveats for our analogy:} Slepian-Wolf assumes lossless compression, i.i.d. data, and known joint distributions. Multi-agent learning violates all three: policies are lossy encoders, experience is sequential and non-stationary, and the ``joint distribution'' of optimal actions depends on the learned policies themselves. Figure~\ref{fig:sw-region} shows the original achievable region with the MARL analog overlaid; the conjecture of Section~\ref{sec:conjecture} adapts the conditional-entropy structure rather than applying the theorem itself.

\begin{figure}[tb]
\centering
\includegraphics[width=0.78\linewidth]{figures/fig1_sw_rate_region.pdf}
\caption{The Slepian-Wolf achievable rate region (shaded) for two correlated sources $X, Y$: independent encoders can achieve the same total rate as joint encoding provided $R_X \geq H(X|Y)$, $R_Y \geq H(Y|X)$, and $R_X + R_Y \geq H(X,Y)$. The conjecture in Section~\ref{sec:conjecture} adapts this conditional-entropy structure to multi-agent learning, with per-agent \emph{capacity proxy} playing the role of channel rate and $H(A_i^* \mid A_{-i}^*)$ replacing $H(X \mid Y)$. The mapping is conceptual; Slepian--Wolf is a lossless block-coding theorem, while MARL is lossy and sequential.}
\label{fig:sw-region}
\end{figure}
...
Three more recent (2024) works further extend the MI-regularization line. \textbf{MIR3}~\cite{liu2024mir3} applies a mutual-information regularizer between agents' histories and actions to bound a robustness gap, framing the IB as an explicit lower bound on worst-case-cooperative-partner reward. \textbf{MADPO}~\cite{madpo2024} \emph{maximizes} policy divergence between agents to enhance heterogeneity in shared-parameter regimes. \textbf{Graph-IB}~\cite{ding2024graphib} applies the information bottleneck to inter-agent communication graphs, treating message bandwidth as a graph-edge capacity constraint --- directly relevant to our Prediction~\ref{pred:communication}. All three share our broad prescription (information-theoretic terms in the MARL objective) without the distributed-source-coding framing or the explicit conditional-entropy budget linking capacity, samples, specialization, communication, and symmetry to a single quantity.
...
This section formalizes our central claim: learned policies are lossy encoders of optimal behavior, and multi-agent coordination is distributed encoding. Figure~\ref{fig:encoder-pipeline} sketches the random-variable flow.

\begin{figure}[tb]
\centering
\includegraphics[width=\linewidth]{figures/fig2_policy_encoder_pipeline.pdf}
\caption{Random-variable flow through the multi-agent system. Nature samples a state $S \sim \rho$; observation functions $\Omega_i$ produce partial observations $O_i$; per-agent encoders (policies $\pi_i$) emit actions $A_i$; the environment evaluates the joint action $A$ and emits reward $R$, which drives gradient updates back to the policies (red, dashed). Encoders share no information at inference time---the only inter-encoder coupling is through the environment's joint evaluation. The latent optimal action distribution $\mathcal{Z}$ (purple) is a conceptual centralized teacher used only for entropy estimation, not at runtime. The environment is an \emph{evaluator}, not a Shannon decoder---it scores $A$ rather than reconstructing $\mathcal{Z}$.}
...
(Section~\ref{sec:conjecture}'s ``Why Conditional Entropy?'' subsection gives concrete patrol-robot scenarios sweeping the spectrum from $H(A_1^* \mid A_2^*) \approx 0$ to $H(A_1^* \mid A_2^*) \approx H(A_1^*)$.)

\textbf{Policy fidelity (KL form)} measures how well the learned policy approximates the teacher:
\[F_{\pi_i} = \mathbb{E}_{S, O_i} \left[ D_{KL}(\mathcal{Z}_i(\cdot|S) \| \pi_i(\cdot|O_i)) \right]\]
Lower is better. In practice we estimate this by sampling states, computing teacher actions, and measuring KL from learned policy outputs.

\textbf{Policy information rate (MI form)} is the symmetric MI quantity we use in the conjecture:
\[R_{\pi_i} \;:=\; I(A_i^*; A_i \mid S),\]
i.e., how much of the optimal action's randomness (given the state) is captured by the learned action. $R_{\pi_i}$ is in bits and is what we compare to $H(A_i^* \mid A_{-i}^*)$ throughout. $F_{\pi_i}$ and $R_{\pi_i}$ are different objects (KL vs.\ MI) and we keep both because each is more natural in different places: $F_{\pi_i}$ for training losses where a teacher is available, $R_{\pi_i}$ for the conjecture's rate inequalities.
...
\paragraph{Strong Form (heuristic only).} A natural idealization---infinite data, exact optimization, stationarity---would suggest tight equality at $r_i = 1$. We do not pursue it: the necessity direction inherits Conjecture~A.2's Lipschitz-in-KL assumption, which is non-trivial for non-myopic MDPs.
...
\textbf{For theory:} If the conjecture holds, fundamental limits would exist for coordination analogous to Shannon limits for communication. Performance is conjectured to show phase transitions near the conditional-entropy threshold, and environments could be characterized by their information structure as well as their state/action spaces.

\textbf{For practice:} Use the estimated conditional entropy to \emph{center} a network-size sweep, not as a closed-form sizing prescription. Design curricula starting with high-correlation tasks. Allow agent capacities to differ when role information requirements differ. Add communication when conditional entropy exceeds practical capacity --- subject to the necessary-but-not-sufficient threshold in Conjecture~A.8.
...
\textbf{Test:} Sweep network sizes from 0.25$\times$ to 4$\times$ estimated minimum; plot performance vs capacity; look for sigmoid transition near the conditional entropy threshold. The conjectured shape is sketched in Figure~\ref{fig:capacity-sigmoid}; the experimental falsifier is smooth growth with no knee.

\begin{figure}[tb]
\centering
\includegraphics[width=0.82\linewidth]{figures/fig4_capacity_sigmoid.pdf}
\caption{Conjectured shape of team reward vs.\ capacity for Prediction~\ref{pred:capacity}. Four capacity proxies (raw parameter count, gzip-compressed weight bits, PAC-Bayes KL, and effective parameters after magnitude pruning) are expected to yield slightly different knee locations near the proxy-implied threshold $C^* \approx H(A_i^* \mid A_{-i}^*) / (\alpha b)$. The curves are illustrative, not measured; the protocol in Section~\ref{sec:experiments} tests whether any knee exists across all proxies, and falsifies P1 if reward grows smoothly with capacity.}
\label{fig:capacity-sigmoid}
\end{figure}
...
Coordination harm & Redundancy penalty breaks sync & Use \emph{conditional} MI ($\mid R$ or $\mid S$) at the representation level \\
\bottomrule
\end{tabular}
\end{center}
...
Bucket Brigade places $N{=}4$ agents around a ring of $K{=}10$ houses (we use $K$ rather than $H$ here to avoid clashing with entropy notation). Each turn, fires spread to neighbors with probability $p_\text{spread}$ and ignite spontaneously with probability $p_\text{ignite}$. Each agent observes a local window of houses (radius $r$) and chooses an action from $\{\text{work\_here}, \text{work\_left}, \text{work\_right}, \text{rest}\}$. Working on a burning house extinguishes it with probability $1 - (1-p_\text{solo})^{n_\text{workers}}$; houses burning more than $\tau$ steps are lost. Rewards include a team component for safe houses, an owner bonus, a work cost, and a rest recovery. Twelve canonical scenarios sweep the regime from trivial cooperation to high-conditional-entropy coordination. Full mechanics and scenario configurations are in Appendix~\ref{app:bucketbrigade}.

The design is intentionally small: $N{=}4$ agents, $|\mathcal{A}_i|{=}4$, $|\mathcal{O}_i|$ a low-dimensional vector. This makes oracle policies tractable to compute and the information quantities $H(A_i^*)$, $H(A_i^* \mid A_{-i}^*)$, and $I(A_i^*; A_i \mid S)$ directly estimable from rollouts. Figure~\ref{fig:bucket-brigade} shows the topology and an indicative state.

\begin{figure}[tb]
\centering
\includegraphics[width=0.72\linewidth]{figures/fig3_bucket_brigade.pdf}
\caption{Bucket Brigade topology: $K{=}10$ houses around a ring, $N{=}4$ agents (blue/dark circles). Houses are safe (green), burning ($\blacklozenge$ in red), or ruined (gray). Each agent observes a local window of radius $r$; the orange band shows agent~1's window with $r=2$. Orange arrows indicate fire spread to adjacent houses with probability $p_\text{spread}$. The action set $\{\textsc{work\_here}, \textsc{work\_left}, \textsc{work\_right}, \textsc{rest}\}$ combined with stochastic fire dynamics produces a tunable conditional-entropy structure across the twelve canonical scenarios (Appendix~\ref{app:bucketbrigade}).}
\label{fig:bucket-brigade}
\end{figure}
...
\textbf{On the ``this is just regularization'' objection.} A fair version of the objection is that ROMA, RODE, PMIC, IMAC, and MIR3 already practice information-theoretic regularization without Slepian--Wolf framing. Our response is that the lens provides a single quantity ($H(A_i^*\mid A_{-i}^*)$) to which capacity, samples, specialization, communication, and symmetry all reduce, plus an explicit prescription (condition on task signal) that addresses PMIC's failure mode. Whether that unification is worth its expository overhead is for the empirical work to decide.

\textbf{On the ``this is just rate--distortion'' objection.} An information-theory--leaning reviewer may note that the conjecture is essentially a rate--distortion statement: an agent's policy needs rate at least $H(A_i^*\mid A_{-i}^*)$ to reconstruct $A_i^*$ within tolerance $\epsilon$. We agree, and consider sequential rate--distortion with side information the formal home for a future tight version of the conjecture; the Slepian--Wolf framing buys explanatory clarity (the conditional structure between agents) more than novel formalism. Wyner--Ziv~\cite{wyner1976} is the closest classical \emph{lossy} result; the appropriate sequential analog is the nonanticipative / directed-information rate--distortion theory developed by Tatikonda and Mitter~\cite{tatikonda2004} and extended by Charalambous and Stavrou~\cite{charalambous2017}, which would need adaptation to multi-agent and learned-source settings. We leave this formal extension open.
...
\bibitem{tatikonda2004} Tatikonda, S.\ \& Mitter, S.\ (2004). Control under communication constraints. \emph{IEEE Trans.\ Automatic Control}, 49(7):1056--1068.
\bibitem{charalambous2017} Charalambous, C.D.\ \& Stavrou, P.A.\ (2017). Directed information on abstract spaces: properties and variational equalities. \emph{IEEE Trans.\ Information Theory}, 62(11):6019--6052.
...
\bibitem{liu2024mir3} Liu, Y., Chen, S., Wang, Y., Mguni, D.H., Hao, J., \& An, B.\ (2024). Robust multi-agent reinforcement learning by mutual information regularization (MIR3). \emph{IEEE TNNLS}; arXiv:2310.09833.
\bibitem{ding2024graphib} Ding, S., Du, W., Ding, L., Guo, L., Zhang, J., \& An, B.\ (2024). Robust multi-agent communication with graph information bottleneck optimization. \emph{IEEE TPAMI}.
\bibitem{madpo2024} Dou, H., Dang, L., Luan, Z., \& Chen, B.\ (2024). Measuring mutual policy divergence for multi-agent sequential exploration (MADPO). \emph{NeurIPS}; OpenReview \texttt{xvYI7TCiU6}.
...
\paragraph{Proposition P.1 (Properties of $\mathcal{Z}$).} The latent optimal action distribution satisfies:
\begin{enumerate}
...
\paragraph{Lemma A.1 (Entropy Decomposition).} The joint entropy of optimal actions decomposes as
\[H(A_1^*, \ldots, A_N^*) \;=\; \sum_{i=1}^N H(A_i^* \mid A_1^*, \ldots, A_{i-1}^*).\]
\emph{Proof.} The chain rule of entropy applied to the random variables $A_i^* \sim \mathcal{Z}_i(\cdot \mid S)$. $\blacksquare$
...
\paragraph{Conjecture A.3 (Achievability).} There exist policies $\{\pi_i\}_{i=1}^N$ with
\[R_{\pi_i} \;=\; H(A_i^* \mid A_{-i}^*) + o(1)\]
achieving $J(\pi) \to J^*$ as the number of samples $n \to \infty$.

\emph{Heuristic motivation.} Conjecture~A.3 is the achievability companion of the necessity conjecture A.2 and inherits the same load-bearing assumption: $J$ is Lipschitz in policy KL with constant $L$, so that
\[J^* - J(\pi) \;\leq\; L \sum_i \mathbb{E}_S\!\left[ D_\text{KL}\bigl(\mathcal{Z}_i(\cdot \mid S) \,\|\, \pi_i(\cdot \mid O_i)\bigr) \right].\]
Given that assumption, achievability needs two further steps that we do not develop here:
\begin{enumerate}
    \item A \emph{concentration result}: that a maximum-likelihood estimator $\hat{\pi}_i$ trained on $(O_i, A_i^*)$ pairs from teacher rollouts has $\mathbb{E}[D_\text{KL}(\mathcal{Z}_i \,\|\, \hat{\pi}_i)] \to 0$ at a rate compatible with the conditional entropy --- non-trivial under non-i.i.d., non-stationary trajectories, but plausible for the discrete Bucket Brigade setting with a fixed teacher.
    \item A \emph{coupling step}: that the per-agent KL decrease aggregates through the Lipschitz inequality above to drive $J^* - J(\pi) \to 0$. This is straightforward given the assumption but inherits its strength.
\end{enumerate}
Together with Conjecture~A.2 (necessity), Conjecture A.3 gives a sketch of the two-sided picture: if Lipschitz-in-KL holds, rate $\approx H(A_i^* \mid A_{-i}^*)$ is both necessary and sufficient. We do not claim either direction is a theorem; both rest on the same assumption that is hard to verify for general MDPs.
...
\paragraph{Definition A.14 (Inter-Policy Redundancy).} The relevant redundancy between agents $i$ and $j$ is the \emph{conditional} mutual information between their representations given the task-relevant signal:
\[\mathcal{R}_{ij} \;:=\; I(\hat{Z}_i; \hat{Z}_j \mid R) \qquad\text{(training-time)}\]
or, equivalently for state-conditioned analysis,
\[\mathcal{R}^{S}_{ij} \;:=\; I(\pi_i; \pi_j \mid S) \qquad\text{(eval-time)}.\]
Both exclude the part of agent-agent MI that the task itself induces; the unconditional $I(\pi_i; \pi_j)$ from v1 conflates the two and we record it only as a diagnostic. PMIC~\cite{li2022pmic} documents the practical failure mode of penalizing the unconditional form.

\paragraph{Proposition P.4 (Lagrangian framing).} When per-agent capacity $|\theta_i|$ is constrained, the Lagrangian relaxation of the constrained problem has the form
...
\[\text{Clarity}_i \;=\; \frac{H(A_i^*) - H(A_i^* \mid A_{-i}^*)}{H(A_i^*)} \;=\; \frac{I(A_i^*; A_{-i}^*)}{H(A_i^*)}.\]
This measures how much of agent $i$'s optimal behavior is determined by teammates'.

\paragraph{Proposition P.2 (Specialization Metrics).} High specialization corresponds to:
\begin{enumerate}
    \item Low redundancy: $\mathcal{R}_{ij} \to 0$ for all $i \neq j$.
    \item High role clarity: $\text{Clarity}_i \to 1$.
    \item Efficient encoding: $R_{\pi_i} \approx H(A_i^* \mid A_{-i}^*)$.
\end{enumerate}
...
\paragraph{Proposition P.5 (Convergence to local optimum).} Standard stochastic-approximation arguments (Robbins--Monro with $\alpha_t = O(1/\sqrt{t})$, bounded rewards, $L$-Lipschitz policies, and unbiased, bounded-variance gradient estimators) yield almost-sure convergence to a local optimum of
...
For continuous action spaces $\mathcal{A}_i = \mathbb{R}^d$, replace discrete entropy with differential entropy. Let $A_i^*$ have density $\mathcal{Z}_i(\cdot \mid s)$; then
\[h(A_i^* \mid S=s) \;=\; -\int_{\mathbb{R}^d} \mathcal{Z}_i(a \mid s) \log \mathcal{Z}_i(a \mid s)\, da.\]

\paragraph{Proposition P.3 (Continuous Capacity Bound).} For continuous actions with bounded support $[-M, M]^d$,
\[|\theta_i|_{\min} \;\approx\; \frac{h(A_i^* \mid A_{-i}^*) + d\log(2M)}{\alpha \cdot b}.\]
...
Convergence (P.5) & Local optimum under SA conditions & Standard SA argument \\
\bottomrule
\end{tabular}
\end{center}
...
Environment transition $+$ reward & Joint evaluator (analog of decoder) \\
...
\begin{itemize}
    \item $H(A_i^*)$, $H(A_i^* \mid A_{-i}^*)$ --- entropy of optimal actions.
    \item $R_{\pi_i} = I(A_i^*; A_i \mid S)$ --- policy information rate.
    \item $I(\hat{Z}_i; \hat{Z}_j \mid R)$ --- conditional redundancy between agents' representations (the quantity Section~\ref{sec:algorithm}'s objective penalizes).
\end{itemize}

\subsection{Experimental Objectives}

\begin{enumerate}
    \item \textbf{Test the Distributed Learning Conjecture.}
    \begin{itemize}
        \item Train decentralized agents of varying network capacity.
        \item Measure $R_{\pi_i}$ and $H(A_i^* \mid A_{-i}^*)$.
        \item Test whether coordination quality rises once $R_{\pi_i} \gtrsim H(A_i^* \mid A_{-i}^*)$.
    \end{itemize}
    ...
    \begin{itemize}
        \item Monitor $I(\hat{Z}_i; \hat{Z}_j \mid R)$ over training.
...
\midrule
$H(A_i^*)$ & Empirical entropy of oracle actions \\
$H(A_i^* \mid A_{-i}^*)$ & Conditional entropy given others' oracle actions \\
$R_{\pi_i} = I(A_i^*; A_i \mid S)$ & MI between policy actions and oracle actions \\
$I(\hat{Z}_i; \hat{Z}_j \mid R)$ & Conditional redundancy between agents' representations \\
...
\midrule
E1: Capacity Sweep & Vary network size & Performance plateau at $\approx H(A_i^* \mid A_{-i}^*)$ \\
E2: Correlation Ablation & Mask observations & Degraded reward $\propto$ reduced $I(O_i; O_j)$ \\
E3: Specialization Emergence & Train under social dilemma & $I(\hat{Z}_i; \hat{Z}_j \mid R)$ decreases over training \\
E4: Communication Threshold & Enable honest signals & Improves when $H(A_i^* \mid O_i) \gg H(A_i^* \mid A_{-i}^*)$ \\
\bottomrule
\end{tabular}
\end{center}
...
\paragraph{1. Marginal Entropy: $H(A_i^*)$.}
\begin{itemize}
...
\paragraph{2. Conditional Entropy: $H(A_i^* \mid A_{-i}^*)$.}
\begin{itemize}
...
\paragraph{3. Conditional Inter-Policy MI: $I(\hat{Z}_i; \hat{Z}_j \mid R)$.}
\begin{itemize}
    \item Measures redundancy between learned representations beyond what the task itself requires.
    \item High values indicate agents encoding overlapping non-task information.
    \item Penalized by the regularizer in Section~\ref{sec:algorithm}. The unconditional $I(\pi_i; \pi_j)$ is reported only as a diagnostic, since it conflates task-required correlation with wasteful redundancy.
\end{itemize}

\paragraph{4. Policy Information Rate: $R_{\pi_i} = I(A_i^*; A_i \mid S)$.}
\begin{itemize}
    \item Measures how much of the optimal action's randomness given the state is captured by the learned action.
    \item Should approach $H(A_i^* \mid A_{-i}^*)$ for good coordination.
...
\textbf{Step 1.} Express conditional entropy for Gaussians. For jointly Gaussian random variables, the conditional differential entropy is
\[h(A_i^* \mid A_{-i}^*) \;=\; \tfrac{1}{2} \log\bigl((2\pi e)^{d_i} \det(\Sigma_{i\mid -i})\bigr),\]
where $\Sigma_{i\mid -i}$ is the conditional covariance matrix and $d_i$ the dimension of $A_i^*$.

\textbf{Step 2.} Relate network capacity to rank. For a linear transformation $W_i$, the maximum preserved information from $O_i$ is
\[I(O_i; W_i O_i) \;\leq\; \mathrm{rank}(W_i) \cdot \log \sigma_{\max}(W_i).\]

\textbf{Step 3.} Apply the data-processing inequality. Since $A_i^* \to O_i \to W_i O_i$ forms a Markov chain (the policy is a deterministic function of $O_i$ in the linear case),
\[I(A_i^*; A_i \mid S) \;\leq\; I(A_i^*; O_i) \;\leq\; I(O_i; W_i O_i).\]

\textbf{Step 4.} Combine with Conjecture~A.2: for $\epsilon$-optimal performance,
\[I(A_i^*; A_i \mid S) \;\gtrsim\; h(A_i^* \mid A_{-i}^*) - \delta(\epsilon).\]
Therefore,
\[\mathrm{rank}(W_i) \cdot \log \sigma_{\max}(W_i) \;\geq\; h(A_i^* \mid A_{-i}^*) - \delta(\epsilon),\]
which is the rank--spectral-norm coupling claimed by Proposition E.1. $\square$
...
\[H(A_i^* \mid b_i, A_{-i}^*) \;=\; \mathbb{E}_{S \sim b_i}\!\left[ H(A_i^* \mid S, A_{-i}^*) \right].\]

\paragraph{Conjecture E.4 (POMDP Capacity).} In POMDPs, required capacity is conjectured to increase by a belief-entropy term:
\[C^*_{\text{POMDP}} \;\approx\; H(A_i^* \mid A_{-i}^*) + \beta \cdot H(S \mid O_i),\]
where $\beta \in [0,1]$ depends on the mixing time of the belief MDP. Like its MDP counterpart (Proxy~A.6), this is a sizing heuristic derived from the conditional-entropy intuition rather than a derivation; the belief-MDP analysis required to justify the $\beta$ factor is not developed here.
...
\[\mathcal{R}_{\pi_i}(t) \;=\; \lim_{\Delta t \to 0} \frac{I(A_i^*(t); A_i(t))}{\Delta t}.\]

\paragraph{Conjecture E.5 (Continuous-Time Capacity).} The minimum capacity for continuous-time coordination is conjectured to be
\[C^*_{\text{continuous}} \;\approx\; \int_0^T \mathcal{R}_{A_i^* \mid A_{-i}^*}(t)\, dt,\]
where $\mathcal{R}_{A_i^* \mid A_{-i}^*}(t)$ is the conditional information rate. A formal version would need a continuous-time analog of Conjecture~A.2 with appropriate regularity assumptions, which we do not develop.
